...
class WindowAggregator:
    def __init__(
        self, window_seconds: int, on_window_complete: Callable[[FeatureVector], None]
    ):
    ...
    def ingest(self, event: RawEvent):
        with self._lock:
            ...
            if event.event_type == EventType.execve:
                acc.num_execve += 1
                if event.filename:
                    acc.distinct_files.add(event.filename)
    ...
    def _flush(self, key: WindowKey):
        ...
        duration_s = max(
            (acc.last_event_ns - acc.window_start_ns) / 1_000_000_000, 0.001
        )
        ...
        vector = FeatureVector(
            ...
            window_end_ns=acc.last_event_ns,
    ...
    def _reap_loop(self):
        while not self._stop.is_set():
            ...
            with self._lock:
                stale_keys = [
                    k
                    for k, acc in self._accumulators.items()
                    if (now_ns - acc.last_event_ns) / 1_000_000_000
                    >= self.window_seconds
                ]
                for k in stale_keys:
                    self._flush(k)
